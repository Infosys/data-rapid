...
/**
 * @Description Generates a list of first names from the database.
 */
public class FirstName implements DataGenerator {
    ...
    public List<String> getPersonalInfoFromDB(int noOfValues) {
        ...
        if (interm == 0) {
            ...
            for (int p = 1; p <= noOfValues; p++) {
                ...
                list.add(emailAddressBeans.get(p).getFirstName());
            ...
            for (int i = 0; i < interm; i++) {
                ...
                for (int j = 0; j < 5000; j++) {
                    list.add(emailAddressBeans.get(j).getFirstName());

                }
            }
            for (int i = 0; i < remains; i++) {
                list.add(emailAddressBeans.get(i).getFirstName());
